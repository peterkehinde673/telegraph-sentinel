...
import { handleMinerTvlLookup } from './tvl_oracle';
import { handleMinerWebSearch } from './web_search';
...
app.all('/api/v1/miner/tvl', handleMinerTvlLookup);
app.all('/api/v1/miner/web-search', handleMinerWebSearch);
...
app.post('/api/v1/miner/onchain/encode-register', (req: Request, res: Response) => {
  ...
  res.json({ ...encoded, chainIdentifier: TELEGRAPH_REGISTRY_CONFIG.caip2, chainId: TELEGRAPH_REGISTRY_CONFIG.chainId });
});
...
app.post('/api/analyze', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (useFixture) {
      ...
      if (fs.existsSync(fixturePath)) signals = JSON.parse(fs.readFileSync(fixturePath, 'utf8')).signals;
    }
    ...
    if (signals.length === 0) {
      ...
      signals = [
        { miner_id: 207, miner_name: 'CoinGecko', intent: 'CRYPTO_PRICE', status: livePrice ? 'success' : 'unavailable', timestamp: nowIso, risk_signal: priceRisk, confidence: priceConf, data: livePrice ? { price_usd: livePrice.price, change_24h_pct: livePrice.change24h } : { error: 'Upstream price unavailable' }, verification: { verified: false, status: 'UNVERIFIED', minerId: 207 }, payment: { settled: false, status: 'NOT_REQUIRED' } },
        { miner_id: 301, miner_name: 'TVL Oracle', intent: 'TVL_LOOKUP', status: 'success', timestamp: nowIso, risk_signal: 15.0, confidence: 90.0, data: { protocol: asset, status: 'Healthy Liquidity' }, verification: { verified: false, status: 'UNVERIFIED', minerId: 301 }, payment: { settled: false, status: 'NOT_REQUIRED' } },
        { miner_id: 202, miner_name: 'Tavily', intent: 'WEB_SEARCH', status: 'success', timestamp: nowIso, risk_signal: 10.0, confidence: 88.0, data: { security_exploits_found: 0, sentiment: 'Positive' }, verification: { verified: false, status: 'UNVERIFIED', minerId: 202 }, payment: { settled: false, status: 'NOT_REQUIRED' } }
      ];
    }
    ...
    if (risk_score <= 30.0) { decision = 'APPROVE'; reason_codes.push('RISK_WITHIN_NORMAL_BOUNDS'); }
    else if (risk_score <= 60.0) { decision = 'REVIEW'; reason_codes.push('ELEVATED_VOLATILITY_REVIEW'); }
    else { decision = mode === 'PROTECT' ? 'BLOCK' : 'HIGH_RISK_REVIEW'; reason_codes.push('HIGH_RISK_THRESHOLD_EXCEEDED'); }
    ...
    const result = { analysis_id: crypto.randomUUID(), asset, mode, action_type, risk_score, confidence_score, decision, reason_codes, created_at: nowIso, signals, evidence, verification_metadata: { network: 'eip155:84532', x402_status: 'NOT_REQUIRED', verified: false } };
...
app.get('/api/status', (_req: Request, res: Response) => {
  res.status(200).json({ gateway: 'active', miners: [
...
app.get('/api/analyses', async (_req: Request, res: Response, next: NextFunction) => { try { res.status(200).json({ analyses: await listAnalyses(25) }); } catch (err) { next(err); } });
app.get('/api/watch', (_req: Request, res: Response) => res.status(200).json({ rules: getWatchRules() }));
app.post('/api/watch', (req: Request, res: Response) => {
  ...
  if (!asset) return res.status(400).json({ error: 'Asset is required for watch rule', status: 400 });
  return res.status(201).json(addWatchRule({ asset: asset.toString().toUpperCase(), mode, riskThreshold: Number(riskThreshold), confidenceThreshold: Number(confidenceThreshold), intervalMinutes: Number(intervalMinutes) }));
});
app.delete('/api/watch/:id', (req: Request, res: Response) => {
  const deleted = deleteWatchRule(req.params.id);
  if (!deleted) return res.status(404).json({ error: 'Watch rule not found', status: 404 });
  return res.status(200).json({ status: 'deleted', id: req.params.id });
});

app.get('/health', (_req: Request, res: Response) => res.status(200).json({ status: 'healthy', service: 'sentinel-node-gateway', timestamp: new Date().toISOString() }));
app.all('/api/*', (req: Request, res: Response) => res.status(404).json({ error: `API route '${req.method} ${req.originalUrl}' not found`, status: 404 }));
app.use((err: any, _req: Request, res: Response, _next: NextFunction) => res.status(err.status || 500).json({ error: err.message || 'Internal Server Error', status: err.status || 500 }));

const PORT = process.env.PORT ? parseInt(process.env.PORT, 10) : config.port || 4000;
server.listen(PORT, '0.0.0.0', () => console.log(`[Telegraph Sentinel] Server active on port ${PORT}`));
export { app, server };
